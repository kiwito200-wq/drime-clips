  ...
  const handleDrimeFileSelect = async (drimeFile: any, blob?: Blob) => {
    try {
      ...
      if (response.ok) {
        ...
        const error = await response.json()
        alert(error.error || (locale === 'fr' ? 'Échec de la création depuis Drime' : 'Failed to create from Drime'))
      }
    } catch (error) {
      console.error('Error creating envelope from Drime:', error)
      alert(locale === 'fr' ? 'Échec de la création depuis Drime' : 'Failed to create from Drime')
    }
  }
