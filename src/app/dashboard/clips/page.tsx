                ...
                <div className="space-y-4">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500">
                          {isPublic ? 'N\'importe qui peut voir' : 'Vidéo privée'}
                        </p>
                      </div>
                    </div>
                    <button
                      onClick={() => setIsPublic(!isPublic)}
                      className={`relative w-12 h-7 rounded-full transition-colors ${isPublic ? 'bg-[#08CF65]' : 'bg-gray-300'}`}
                    >
                      <motion.div 
                  ...
                  {isPublic && (
                    <div className="flex items-center gap-2">
                      <div className="flex-1 px-4 py-3 bg-gray-50 rounded-xl text-sm text-gray-600 truncate border border-gray-200">
            ...
            {activeTab === 'share' && (
              <div className="px-5 pb-5 flex justify-end gap-3">
                ...
                <button
                  onClick={handleSave}
                  disabled={saving}
                  className="px-5 py-2.5 text-sm font-medium text-white bg-gray-900 rounded-xl hover:bg-gray-800 disabled:opacity-50 transition-colors"
                >
      ...
      <Link href={`/v/${video.id}`} className="block relative aspect-video bg-gray-900">
        ...
        <div className={`absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent flex items-end justify-end p-2 gap-1.5 transition-opacity duration-200 ${isHovering && !video.hasActiveUpload ? 'opacity-100' : 'opacity-0'}`}>
          ...
          <div className="relative" ref={menuRef}>
            ...
            {showMenu && (
              <div className="absolute right-0 bottom-full mb-2 w-40 bg-white rounded-xl shadow-lg border border-gray-200 py-1 z-50">
                <Link
                  href={`/v/${video.id}`}
